...
*   `SigLayer`: 시그모이드 활성화 함수를 구현합니다.
*   `ReluLayer`: ReLU (활성화 함수)와 변형된 형태 (음수 입력에 0.1x)를 구현합니다.
*   `StepLayer`: 스텝 함수를 구현하여 최종 출력을 이진 분류합니다.

백프로파게이션(역전파) 알고리즘은 주어진 XOR 데이터셋에 대해 신경망의 가중치와 편향을 훈련하는 데 사용됩니다.
## 사용법
